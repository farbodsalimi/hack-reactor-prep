...
assertObjectsEqual(
  objectA,
  objectB,
  'should pass when given two identical objects'
);
...
function assert(actual, testName) {
  if (actual !== true) {
    console.log(
      'FAILED [' + testName + '] Expected "' + expected + '" to be true.'
    );
  } else {
    console.log('PASSED [' + testName + ']');
  }
...
function testFailureCaseAssertObjectsEqual() {
  ...
  assert(
    trappedMsg.includes('FAILED'),
    'should fail when given two different objects'
  );
}
...
assertObjectsEqual(
  addFullNameProp(actualObject),
  expectedObject,
  'should add a fullName'
);
